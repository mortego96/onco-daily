...
import { fetchFdaApprovals } from "./fda.js";
...
async function main() {
  ...
  // --- Fuente adicional: aprobaciones de la FDA relevantes para tus tumores ---
  if (cards.length < settings.maxCards) {
    process.stdout.write(`\n[FDA · aprobaciones] buscando… `);
    try {
      const approvals = await fetchFdaApprovals(settings.daysBack);
      const freshApprovals = approvals.filter((a) => !seen.has(a.pmid));
      if (!freshApprovals.length) {
        console.log("sin novedades.");
      } else {
        console.log(`${freshApprovals.length} novedad(es).`);
        const fdaTopic = { id: "fda", tag: "Aprobación FDA" };
        for (const article of freshApprovals) {
          if (cards.length >= settings.maxCards) break;
          try {
            const card = await summarizeArticle(article, fdaTopic, {
              apiKey: process.env.GEMINI_API_KEY,
              model: settings.model,
            });
            cards.push(card);
            ...
            console.log(`  ✗ no se pudo resumir: ${e.message}`);
          }
          await sleep(GEMINI_DELAY);
        }
      }
    } catch (e) {
      console.log(`error al leer el feed de la FDA: ${e.message}`);
    }
  }
